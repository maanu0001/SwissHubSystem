...
- **Berechtigung im Dienst, nicht in der Oberfläche.** `spielwahl.games.manage`
  wird sowohl von der Server Action als auch vom Katalogdienst selbst geprüft.
  Das ist keine Doppelung aus Unsicherheit: der Dienst hat mehrere Aufrufer und
  darf sich nicht darauf verlassen, dass jemand vorher nachgesehen hat.
- **Lesen ist frei, Schreiben nicht.** Jedes Mitglied darf aus dem Katalog
  auswählen - er ist die Grundlage von Turnieren, Clips und Runden. Ändern darf
  nur, wer die Berechtigung hat.
- **Eine Cover-Adresse muss `https` sein.** Eine `javascript:`- oder
  `data:`-Adresse stünde sonst in einem `src`-Attribut, und dort bedeutet sie
  etwas anderes als „Bild". Ein hochgeladenes Cover liegt ausserhalb des
  statisch bedienten Verzeichnisses; der Content-Type wird beim Ausliefern fest
  gesetzt, damit eine hochgeladene Datei nie als HTML oder Skript ankommt.
- **Plattformen gegen eine Erlaubnisliste**, nicht frei. Ein Filter funktioniert
  nur, solange „PC" nicht auch „pc" und „PC / Steam" heisst.
- **Gelöscht wird nicht.** An einem Spiel hängen Turniere, Clips und vergangene
  Runden. Archivieren nimmt es aus jeder Auswahl und lässt alles gültig, was
  darauf zeigt - eine Verwaltungsaktion von heute schreibt keine Vergangenheit um.
